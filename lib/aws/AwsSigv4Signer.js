...
function AwsSigv4Signer(opts) {
  ...
  function buildSignedRequestObject(request = {}) {
    ...
    if (request.service === 'aoss') {
      request.headers['x-amz-content-sha256'] = 'UNSIGNED-PAYLOAD'
      body = request.body
      if (body !== undefined) delete request.body
		  contentLength = request.headers['content-length']
      if (contentLength !== undefined) delete request.headers['content-length']
    }

		request = aws4.sign(request, credentialsState.credentials)

		if (body !== undefined) request.body = body
		if (contentLength !== undefined) request.headers['content-length'] = contentLength
